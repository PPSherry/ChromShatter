...
namespace torali
{
  ...
  scanPEandSR(TConfig const& c, TValidRegion const& validRegions, std::vector<StructuralVariantRecord>& svs, std::vector<StructuralVariantRecord>& srSVs, TSRStore& srStore, TSampleLib& sampleLib, int sampleIdx = -1)
  {
    ...
    for(unsigned int file_c = 0; file_c < c.files.size(); ++file_c) {
      samfile[file_c] = sam_open(c.files[file_c].string().c_str(), "r");
      hts_set_fai_filename(samfile[file_c], c.genome.string().c_str()); // allocate the reference index to each samfile
      idx[file_c] = sam_index_load(samfile[file_c], c.files[file_c].string().c_str());
    }
    bam_hdr_t* hdr = sam_hdr_read(samfile[0]); // read the first header

    // Split-read records
    typedef std::vector<SRBamRecord> TSRBamRecord;
    typedef std::vector<TSRBamRecord> TSvtSRBamRecord; // used for different types of SVs
    ...
    typedef std::vector<TBamRecord> TSvtBamRecord; // used for different types of SVs
...
#pragma omp parallel for default(shared)
	...
	// Read alignments
    // iterate over the valid regions
	for(typename TChrIntervals::const_iterator vRIt = validRegions[refIndex].begin(); vRIt != validRegions[refIndex].end(); ++vRIt) {
	  hts_itr_t* iter = sam_itr_queryi(idx[file_c], refIndex, vRIt->lower(), vRIt->upper());
	  bam1_t* rec = bam_init1();
	  ...
	  while (sam_itr_next(samfile[file_c], iter, rec) >= 0) {
	    ...
	    // Parse the CIGAR
	    const uint32_t* cigar = bam_get_cigar(rec); // get the cigar of current alignment record
...
#pragma omp critical
    ...
    // Track split-reads
    // srBR.size() is the number of SV types
    for(uint32_t svt = 0; svt < srBR.size(); ++svt) {
      for(uint32_t i = 0; i < srBR[svt].size(); ++i) {
	// Read assigned? (means assigned to a PE SV， as some SR SVs are not assigned to any PE SV)
  ...
  mergeSort(std::vector<StructuralVariantRecord>& pe, std::vector<StructuralVariantRecord>& sr) {
	...
	// Precise duplicates (SR supported SVs have corresponding PE SVs)
